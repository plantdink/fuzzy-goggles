...
import { fakeShotgunQuery } from '../lib/testUtils';
import { SINGLE_WEAPON_QUERY } from '../components/SingleWeapon';
import SingleShotgunPage from '../pages/weapons/shotgun/[id]';

afterEach(cleanup);

const fakeQuery = fakeShotgunQuery();
...
describe('<SingleShotgunPage />', () => {
  it('displays loading when the page first renders', async () => {
    const { container, debug } = render(
      <>
        <MockedProvider mocks={mocks}>
          <SingleShotgunPage query={{ id: `${fakeQuery.id}` }} />
        </MockedProvider>
      </>
    );
  ...
  it('loads the data and renders both components with data correctly for a single shotgun page', async () => {
    ...
    const weaponClassNotes = await screen.findByTestId('testShotgunNotes');
