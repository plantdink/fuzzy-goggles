...
import { fakeSMGQuery } from '../lib/testUtils';
import { SINGLE_WEAPON_QUERY } from '../components/SingleWeapon';
import SingleSubMachineGunPage from '../pages/weapons/subMachineGun/[id]';

afterEach(cleanup);

const fakeQuery = fakeSMGQuery();
...
describe('<SingleSubMachineGunPage />', () => {
  it('displays loading when the page first renders', async () => {
    const { container, debug } = render(
      <>
        <MockedProvider mocks={mocks}>
          <SingleSubMachineGunPage query={{ id: `${fakeQuery.id}` }} />
        </MockedProvider>
      </>
    );
  ...
  it('loads the data and renders both components with data correctly for a single sub machine gun page', async () => {
    ...
    const weaponClassNotes = await screen.findByTestId('testSMGNotes');
